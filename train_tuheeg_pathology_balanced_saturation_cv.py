...
from braindecode.models import Deep4Net,ShallowFBCSPNet,EEGNetv4, TCN # TIDNet
...
def train_TUHEEG_pathology(model_name,
                     ...
                     cuda = True,
                     shuffle_folds= False,
                     only_last_fold_valid = True,
                     only_fold_x_valid= None,
                     ab_ifold = None,
                     ):
    ...
    with io.capture_output() as captured:
        train_complete = load_concat_dataset(train_folder, preload=False, ids_to_load=ids_to_load_train)

    end = time.time()

    print('finished loading preprocessed trainset ' + str(end-start))

    target = train_complete.description['pathological'].astype(int)
    ...
    n_recs = len(train_complete.datasets)
    cv = KFold(n_splits=n_splits_valid, shuffle=shuffle_folds) # preserving the percentage of samples for each class. 
 
    dirSave = result_folder + model_name + '/' + 'seed'+str(seed) 
    ...
    all_results = list()
    all_results_eval = list()
    ##### choose last fold     
    for ifold, (train_subind, valid_ind) in enumerate(cv.split(range(n_recs))):

        assert len(np.intersect1d(train_subind, valid_ind)) == 0, (
            "train and valid set overlap!")
        print('ifold '+ str(ifold))

        if only_last_fold_valid:
            assert only_fold_x_valid== None, ('only_fold_x_valid must be None if only_last_fold_valid is True')
            if ifold < (cv_split.n_splits - 1):
                continue

        if only_fold_x_valid is not None:
            assert only_last_fold_valid == False, ('only_last_fold_valid must be False if only_fold_x_valid is not None')
            if ifold != only_fold_x_valid:
                continue
        ...
        train_set = train_complete.split(train_subind)['0']

        valid_set = train_complete.split(valid_ind)['0']


        model_number=ifold

    ####Training Loop
        for (i, n_epochs, batch_size, 
                lr, weight_decay, drop_prob ) in product(
                    range(N_REPETITIONS), N_epochs,Batch_size, 
                            Lr, Weight_decay, Drop_prob):
            ...
            save_name =model_name   +'_trained_' +  str(task_name)+ '_' + str(ifold)+ '_ifold_' 
...
## define Data
            ...
            with io.capture_output() as captured:
                 window_valid_set = create_fixed_length_windows(valid_set,
                                                                start_offset_samples=0,
                                                                stop_offset_samples=None,preload=True,
                                                                window_size_samples=input_window_samples,
                                                                window_stride_samples=n_preds_per_input,
            ...
            print('abnormal valid ' + str(len(window_valid_set.description[window_valid_set.description['pathological']==1])))


            print('normal valid ' + str(len(window_valid_set.description[window_valid_set.description['pathological']==0])))
            ...
            pred_win = clf.predict_with_window_inds_and_ys(window_valid_set)
            ...
            y = window_valid_set.description['pathological']
            ...
            # store predictions
            pd.DataFrame.from_dict(a_dict).to_csv(result_path + "predictions_valid_" + str(model_number) +
                                                      ".csv")

            
            deep_preds =  mean_preds_per_trial[:, 0] <=  mean_preds_per_trial[:, 1]
            class_label = valid_set.description['pathological']
            ...
            fig.savefig(result_path + 'confusion_matrix_valid.png')


          
            del model, clf, window_train_set, window_valid_set#, valid_set
